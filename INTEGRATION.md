...
*Integration* is the act of embedding the `CleanroomConcurrency.framework` binary into your project, thereby exposing the API it provides to your code.

CleanroomConcurrency is built as a *Swift framework*, and as such, it has the following base platform requirements:
...
**This is the `swift2.3` branch of CleanroomConcurrency.** It uses **Swift 2.3** and **requires Xcode 8.0 beta 6** (or higher) to compile.
...
- **[Carthage integration](#carthage-integration)** — Explains how to use the [Carthage](https://github.com/Carthage/Carthage) dependency manager for adding CleanroomConcurrency to your project.

- **[Manual integration](#manual-integration)** — Demonstrates the steps for adding CleanroomConcurrency by embedding `CleanroomConcurrency.xcodeproj` within your own Xcode project. (**Note:** Manual integration is a bit more involved than using Carthage).
...
When building a framework for a platform with a simulator, Carthage creates a *universal binary*, allowing `CleanroomConcurrency.framework` to work in the simulator as well as on actual devices.
...
```
github "emaloney/CleanroomConcurrency"
```

### 2. Download & Build using Carthage

The command `carthage update` causes Carthage to download and build CleanroomConcurrency.

By default, `carthage update` builds all platform targets in a project. Normally, this is _not_ what you want; you’ll usually use CleanroomConcurrency on just one platform at a time.
...
Once Carthage is done building CleanroomConcurrency, you can use the `open` command to locate the framework binaries in Finder:
...
Go back to Finder, select the file `CleanroomConcurrency.framework`, and then drag it into the list area directly below *Embedded Binaries*.
...
```
"$PROJECT_DIR"/Carthage/Checkouts/CleanroomConcurrency/BuildControl/bin/stripCarthageFrameworks.sh
```
...
Once you’ve done this, try building your application. If you don’t see any errors, **_you’re all done integrating CleanroomConcurrency!_**

But before you start coding, skip to the [Adding the Swift import](#adding-the-swift-import) section to see how you can import CleanroomConcurrency for use in your Swift code.

## Manual Integration

Manual integration involves embedding `CleanroomConcurrency.xcodeproj` directly in your Xcode project.

This will ensure that CleanroomConcurrency is built with the exact same settings you’re using for your app. You won’t have to fiddle with different settings for different architectures — when you’re running in the simulator, it will work; when you then switch to building for device, it’ll work, too.

You’ll also be able to step into CleanroomConcurrency code directly in the debugger without worrying about `.dSYM` resolution, which is very helpful.

### An Overview of the Process

Manual integration is a bit involved; there are five high-level tasks that you’ll need to perform:

1. Download the CleanroomConcurrency source into your project structure
2. Embed `CleanroomConcurrency.xcodeproj` in your Xcode project
3. Build `CleanroomConcurrency.framework`
4. Add `CleanroomConcurrency.framework` to your application target
...
For our integration examples, we’re going to be showing the top-level `CleanroomConcurrency` directory inside a `Libraries` directory at the root level of of your application’s source.
...
### 1. Download the CleanroomConcurrency source

If you’re already using git for version control, we recommend adding CleanroomConcurrency to your project as a submodule. This will allow you to “lock” your codebase to specific versions of CleanroomConcurrency, making it easier to incorporate new versions on whatever schedule works best for you.

If you’re using some other form of version control of if you’re not using version control at all—*shame on you!*—then you’ll want to *clone* the CleanroomConcurrency repository. We suggest putting the CleanroomConcurrency clone somewhere within your application’s directory structure, so that it is included in whatever version control regimen you’re using.

#### Downloading CleanroomConcurrency as a submodule

> **Important:** Skip this section if you plan to download CleanroomConcurrency using `git clone`.

From within the `Libraries` directory, issue the following commands to download CleanroomConcurrency:

```bash
git submodule add https://github.com/emaloney/CleanroomConcurrency
git submodule update --init --recursive
```

Next, you’re ready to [embed the `CleanroomConcurrency.xcodeproj` project file in your Xcode project](#2-embed-cleanroomconcurrency-in-your-project).

#### Downloading CleanroomConcurrency as a cloned repo

> **Important:** Skip this section if you already performed the tasks outlined in “Downloading CleanroomConcurrency as a submodule” above.

From within the `Libraries` directory, issue the following command to clone the CleanroomConcurrency repository:

```bash
git clone https://github.com/emaloney/CleanroomConcurrency
```

### 2. Embed CleanroomConcurrency in your project

In the Terminal, the command `open CleanroomConcurrency` to open the folder containing the CleanroomConcurrency source in the Finder. This will reveal the `CleanroomConcurrency.xcodeproj` Xcode project and all files needed to build `CleanroomConcurrency.framework`.

Then, open your application in Xcode, and drag `CleanroomConcurrency.xcodeproj` into the Xcode project browser. This will embed CleanroomConcurrency in your project and allow you to add the targets built by CleanroomConcurrency to your project.

### 3. Build CleanroomConcurrency
...
Once a device-based run destination has been selected, select the appropriate build scheme for the target platform: “CleanroomConcurrency-iOS”, “CleanroomConcurrency-OSX”, “CleanroomConcurrency-tvOS” or “CleanroomConcurrency-watchOS”.

Then, select *Build* (⌘B) from the *Product* menu.

Once the build is complete, open `CleanroomConcurrency.xcodeproj` in the project navigator and find the “Products” group. Open that, and right-click on `CleanroomConcurrency.framework`. Select *Show in Finder*. This will open the folder containing the framework binary you just built.

You may see several files in this folder; the one we’re concerned with is:

- `CleanroomConcurrency.framework`
...
So, to make things sane again, you’ll need to make sure Xcode references `CleanroomConcurrency.framework` using a “Relative to Build Products” location.

To do this:

1. Locate `CleanroomConcurrency.framework` in the Xcode project browser
...
Once you’ve done this for each framework, **_you’re all done integrating CleanroomConcurrency!_**

## Adding the Swift import

Once CleanroomConcurrency has been successfully integrated, all you will need to do is add the following `import` statement to any Swift source file where you want to use CleanroomConcurrency:

```swift
import CleanroomConcurrency
```

Want to learn more about CleanroomConcurrency? Check out [the README](https://github.com/emaloney/CleanroomConcurrency/blob/master/README.md).
