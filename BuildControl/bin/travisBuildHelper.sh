...
while [[ $THIS_TRY < $MAXIMUM_TRIES ]]; do
	...
	( set -o pipefail && xcodebuild -project CleanroomConcurrency.xcodeproj -configuration Debug -scheme "CleanroomConcurrency" -destination "$DESTINATION" -destination-timeout 300 $XCODE_ACTION 2>&1 | tee "CleanroomConcurrency-$PLATFORM-$OPERATION.log" | xcpretty )
